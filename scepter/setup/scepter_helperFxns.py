...
def set_vars(default_args: dict, 
             system_args: dict) -> dict:
    '''
...
def save_dict_to_text_file(dictionary: dict, 
                           filename: str, 
                           delimiter: str = '\t'):
    '''
...
def preprocess_txt(file_path: str,
                  ) -> pd.DataFrame:
    '''
...
def duration_check(directory: str,
...
def empty_file_check(directory: str,
...
def empty_dir_check(directory: str,
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for entry in os.listdir(directory):
            ...
            if os.path.isdir(entry_path):  # check if it is a subdirectory
                contents = [item for item in os.listdir(entry_path) if item != '.ipynb_checkpoints']
                if not contents:  # check if the subdirectory is empty
                    if entry_path not in empty_subdirs:  # only append if we don't have it already (we had some duplication issues)
...
def run_complete_check(runname_field: str,
                       runname_lab: str,
                       outdir: str,
    ...
    for runname in [runname_field,runname_lab]:
        if runname == runname_lab:
            include_duration_check_internal = False # don't do duration checks on lab runs
        else:
        ...
        none_emptyDir_tf, empty_dirs, prof_exists, flx_exists = empty_dir_check(dst, omit_ipynb)
        ...
        with open(fn_checkres, 'w') as file:
            file.write(f"*** results of postprocess diagnostic checks\n")
            file.write(f"check1 -- Profile subdir exists: \t{prof_exists}\n")
            file.write(f"check2 -- Flx subdir exists: \t{flx_exists}\n")
            file.write(f"check3 -- Prof and flx dirs are not empty: \t{none_emptyDir_tf}\n")
        ...
        with open(fn_checkres, 'w') as file:
            file.write(f"*** log of postprocess diagnostic checks\n")
            file.write(f"\n")
            file.write(f"check1 -- no output (see check_results.res)\n")
            file.write(f"\n\n")
            file.write(f"check2 -- no output (see check_results.res)\n")
            file.write(f"\n\n")
            file.write(f"check3 -- list of empty output directories\n")
            for empty_dir in empty_dirs:
                file.write(f"{empty_dir}\n")
            file.write(f"\n\n")
            file.write(f"check4 -- list of empty output files\n")
            for empty_file in empty_files:
                file.write(f"{empty_file}\n")
            file.write(f"\n\n")
            file.write(f"check5 -- target versus model duration (years) \n")
...
def aws_copy_to_local(aws_bucket: str,
